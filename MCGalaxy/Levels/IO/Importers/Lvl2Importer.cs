...
namespace MCGalaxy.Levels.IO {
    ...
    public unsafe sealed class Lvl2Importer : IMapImporter
    {
        ...
        public override Level Read(Stream src, string name, bool metadata) {
            using (Stream gs = new GZipStream(src, CompressionMode.Decompress, true)) {
                ...
                for (;;) {
                    ...
                    /* currently unimplemented - TEMP SETUP TOMORROW
                    if (section == 0xCC)
                    {
                        ReadTransientDataSection(lvl, gs);
                        continue; 
                    }
                    end currently unimplemented - TEMP SETUP TOMORROW
                    */
        ...
        static Vec3U16 ReadHeader(Stream gs, byte[] header, out int offset) {
            ...
            if (dims.X == 1875) { // version field, width is next ushort
